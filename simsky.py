...
#
# Kara Kundert (karakundert@gmail.com)
# Dec 2013
...
# makeBeamDifference() {{{1
def makeBeamDifference(pb1="primary-beam-model",pb2="primary-beam-perturbed"):
    """Create file that contains difference between two beam models.

    Arguments:
    pb1='primary-beam-model'
        File name that contains reference beam.
    pb2='primary-beam-perturbed'
        File name that contains beam to subtract from the reference beam.

    Results:
    A file named 'pbdiff' is created that contains the difference.
    """

    # Read reference beam file
    ia.open(pb1)
    pix1 = ia.getchunk()
    ia.close()

    # Read perturbed beam file
    ia.open(pb2)
    pix2 = ia.getchunk()
    ia.close()

    # Create empty image file named 'pbdiff'
    os.system('cp -r '+pb1+' pbdiff')

    # Write the difference into 'pbdiff'
    ia.open("pbdiff")
